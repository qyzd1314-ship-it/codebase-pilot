...
代码仓库：https://github.com/qyzd1314-ship-it/codebase-pilot
...
当前项目已拆分为 `h2` 和 `postgres` 两套数据库 profile：

- `h2`：本地快速调试，默认启用
- `postgres`：正式运行 / 面试演示建议使用

## 默认 profile

默认 profile 为 `h2`，配置在：

- [application.yml](/D:/mycode/codebase-pilot/src/main/resources/application.yml)
...
- [application-h2.yml](/D:/mycode/codebase-pilot/src/main/resources/application-h2.yml)
...
```sql
CREATE DATABASE codebase_agent;
```
...
如果希望展示更接近正式运行环境的配置，建议使用 `postgres` profile。
# Database Profiles Quick Guide

The project now supports two database profiles:

- `h2`: local fast debugging
- `postgres`: formal runtime / interview-demo environment
...
- Use `h2` for zero-dependency local debugging.
- Use `postgres` when you want persistent Repo / CodeChunk / Task / AgentStep / Artifact history.

Why H2 is no longer the formal runtime database:

- H2 is ideal for fast local development, but it is still an embedded development database.
- PostgreSQL is a better fit for validating long text fields, indexes, DDL compatibility, and restart-persistent history.
- For interview demos, PostgreSQL better reflects a real persistence layer.

PostgreSQL persists the project’s core data, including:

- Repo
- CodeChunk
- Task
- AgentStep
- EvidenceRef-related payloads
- Artifact
- Eval-related runtime outputs when persisted in later stages

Acceptance checklist:

1. Start PostgreSQL container
2. Start backend with `postgres` profile
3. Clone Repo
4. Index Repo
5. Create Task
6. Inspect Agent Timeline
7. Inspect Artifact
8. Restart service and confirm history still exists
...
## PgVector (Optional Enhancement)

The project now supports an optional pgvector-based retrieval enhancement for `postgres` profile.

- Default PostgreSQL runtime: [docker-compose.postgres.yml](./docker-compose.postgres.yml)
- Optional pgvector runtime: [docker-compose.pgvector.yml](./docker-compose.pgvector.yml)

Use pgvector when you want to persist code chunk embeddings in PostgreSQL and enable vector retrieval + hybrid search.
It is **not** required for the project to start or run the existing Agent workflow.

### Start plain PostgreSQL
...
- Flyway `V2__init_pgvector_chunk_embeddings.sql` will create `code_chunk_embedding`
- repo indexing will persist chunk embeddings into PostgreSQL
- search can compare `KEYWORD_ONLY`, `VECTOR_ONLY`, and `HYBRID`

When pgvector is not available:

- Flyway will skip vector table initialization
- indexing keeps writing `embedding_json` on `code_chunk`
- search automatically falls back to the existing in-memory cosine similarity path
